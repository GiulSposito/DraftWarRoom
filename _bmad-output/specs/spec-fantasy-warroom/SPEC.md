...
- **CAP-3 — Snake schedule generation**
  - **intent:** System generates the complete pick order for a 12-team, 14-round snake draft.
  - **success:** the schedule has 168 picks and reverses correctly at each round boundary (pick 13 belongs to team 12, pick 24 to team 1).
...
- Only the initial league format is fully validated this epic: 12 teams, 14 rounds, roster QB 1 / RB 2 / WR 2 / TE 1 / FLEX 1 (RB or WR) / K 1 / DST 1, bench 6, Full PPR; draft order entered immediately before the draft; user team and slot configurable in `config.R`.
- The epic is implemented as eight independently reviewable stories in the fixed order defined in `operations.md`; no story implements work belonging to a later story. Human checkpoints are required for stories 1, 3, 5, 6, and 8.
