...
- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/12-grouped-roster-panel.md`
  summary: `app.R`'s `output$roster_table` selects the value currency inline with `val <- if ("vor" %in% names(roster)) roster$vor else roster$points`, duplicating the core's `.warroom_value_of()` (`R/recommendation.R:136-140`). If the core ever changes its value currency, the roster panel's RB1/RB2 and bench ordering would drift out of sync with the slotting that `roster_slots()` (which uses `.warroom_value_of()` internally) produces.
  evidence: Review finding (blind-hunter, story 12). The expression is pre-existing — story 8's `renderTable` for `roster_table` already had it (`app.R:341`) — and story 12 only carries it forward. AGENTS.md forbids duplicated formulas in adapters. The clean fix is to export a small core helper (e.g. `player_value(df)`) and call it from both `app.R` and the core; do it in a story that already owns a value-ordering surface, or a dedicated adapter-cleanup pass.

- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/13-microcopy-and-feedback-pass.md`
  summary: The off-turn recommendation warning ("Voce nao esta na vez -- estes numeros assumem que voce pica agora.") is written out verbatim in two adapters: `app.R`'s `output$recs_note` and `scripts/draft.R:97`. Story 13 rewrote the pick/undo feedback strings to the `EXPERIENCE.md` Voice and Tone table but left this one untouched.
  evidence: Story 13's "Ask First" boundary explicitly deferred it: the phrase is duplicated adapter copy (not a core string), so a shared-copy pass that also re-tones it to Voice and Tone and re-adjudicates its `.recs-note` colour (see the story-9 deferred item about `--danger` vs `--warning`) is its own change. Do both together in a story that already owns the recs-note surface or a dedicated adapter-copy pass. `recommend_players()` still has no on-turn guard (story-6 deferred item), so the note's trigger is also unverified off the auto-show path.

- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/13-microcopy-and-feedback-pass.md`
  summary: The persistent `uiOutput("draft_feedback")` region is a normal-flow sibling immediately below the sticky `.status-strip` (`z-index: 5`, opaque). When the operator scrolls the board / roster / Disponíveis tables, a "persistent" error line scrolls up under the sticky strip and out of view — the region delivers persistence-in-the-reactive-value but not persistence-on-screen, which is the point DESIGN.md:171 ("erros persistem até o operador poder agir") makes.
  evidence: Review finding (blind-hunter, story 13). Story 13's own "Ask First" boundary defers section/region layout to story 14 (panel-grid layout), which establishes the workspace/wide/audit regions and will place this feedback region deliberately — pinning it (a sticky wrapper around the strip + feedback, or the region inside the workspace region's header) belongs there. The `.status-strip` sticky rule is locked by the story-10 loop-1 regression test, so the fix must wrap rather than move it. Until then the region is still visible at the moment of the action (viewport at the top right after a click).

- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/13-microcopy-and-feedback-pass.md`
  summary: The empty-selection click ("Selecione um jogador na busca antes de registrar.") is rendered as `kind = "error"` — a persistent `--danger` red bar that stays until the next pick/undo — where the pre-story-13 code used a transient `type = "warning"`. A premature click is guidance, not a failure; a persistent danger state slightly over-signals it (EXPERIENCE.md Voice and Tone: "evite ... alarmismo").
  evidence: Review finding (blind-hunter, story 13). The spec's frozen Boundaries fixed `kind = "error"` for this case, so changing it to a calmer / non-persistent treatment (or a third `kind = "info"`) needs a spec renegotiation. Low impact: the red clears the instant the operator selects a player and registers. Revisit alongside the feedback-region a11y pass (story 21) or a `kind` taxonomy change.

- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/14-candidate-list-readability-and-click-to-pick.md`
  summary: A fast second click on a smart-list candidate row, landing after `commit_state()` has re-rendered the list but before the operator perceives the change, registers an unwanted extra pick — the row now holds a different player, and `record_pick()` accepts it. `do_pick()` has no in-flight / debounce guard, and the buttons are never disabled. `draft_btn` is not exposed the same way (its `player_choice` still points at the just-drafted id, so a second click hits the "already drafted" branch).
  evidence: Review finding (edge-case-hunter + blind-hunter, story 14). Story 14's frozen Approach chose immediate registration ("o undo da story 13 cobre um clique errado"); Shiny input coalescing plus `observeEvent(ignoreNULL=TRUE)` treating `shinyActionButtonValue == 0` as a non-event already close the within-flush window, and the persistent feedback region shows both "Registrado:" lines so the operator sees the double. The residual after-render window is narrow and undo-recoverable. A real fix (roving tabindex + a single selection-driven register, or disabling rows during commit) belongs with story 22's keyboard interaction model, which reworks the whole candidate-row interaction.

- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/14-candidate-list-readability-and-click-to-pick.md`
  summary: Each candidate row's accessible name is `aria-label="Registrar <jogador>"`, which replaces (not augments) the visible row text — screen-reader and voice-control users lose the rank digit, tier, score and the one-line reason that sighted users read, and voice activation by any visible token other than the player name fails (WCAG 2.5.3 Label in Name).
  evidence: Review finding (blind-hunter, story 14). The frozen Boundaries pin the `aria-label` to exactly "Registrar <jogador>". The 2026-08-31 EXPERIENCE.md Sprint Change reduced ARIA to a floor and routes the smart-list's listbox/option semantics (full accessible name per option: name, position, NFL team, availability, list position, alerts — EXPERIENCE.md "Accessibility Floor") to story 23 (C3). Re-adjudicate the `aria-label` shape there.

- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/14-candidate-list-readability-and-click-to-pick.md`
  summary: Registering a pick by clicking a candidate row re-renders the whole smart list, recreating every `<button>` and dropping keyboard focus to the top of the document. Nothing moves focus to a sensible next target (the new row 1, or the search field).
  evidence: Review finding (blind-hunter, story 14). Pre-existing in spirit for `draft_btn` too. Story 22 (C1) owns the keyboard interaction model — "/" focuses search, up/down move a selection, `Enter` registers — and is where post-pick focus placement should be defined, alongside a roving-tabindex model for the candidate list.

- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/14-candidate-list-readability-and-click-to-pick.md`
  summary: `app.R`'s row-click wiring is `lapply(seq_len(10L), ...)` — 10 fixed observers keyed by rank — which hard-codes the `n = 10L` default of `recommend_players()` in the adapter. If that default is raised, rows 11+ render with hover/aria affordances but are inert on click.
  evidence: Review finding (blind-hunter + verification-gap, story 14). Story 14's frozen Boundaries acknowledge the coupling ("`k` de 1 a 10, casando o `n = 10L` default"). Observers must be created statically at server init, so a reactive count is awkward; a generous fixed cap (e.g. 20) or a shared exported constant for the recommendation list length would decouple it. Low risk while the default holds.

- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/15-all-team-rosters-panel.md`
  summary: The rule that pulls K and DST out of `roster_slots()`'s BENCH into dedicated Titulares slots by position identity lives only in `app.R` (story 12's `roster_panel_ui()`), and story 15 now runs it for all 12 teams. `roster_slots()` in `R/recommendation.R` still returns K/DST as BENCH (mirroring `lineup_value()`), so the display slotting and the core slotting disagree by design and could drift silently if `lineup_value()` semantics change.
  evidence: Review finding (blind-hunter, story 15). Story 12's Spec Change Log (loopback 1) settled K/DST-as-dedicated-slots as a human decision and kept `roster_slots()` unchanged; story 15 amplifies the duplication 12x but adds no new logic. A genuine single slotting path would add an optional flag or a sibling function in `R/recommendation.R` that assigns K/DST slots, so both the operator panel and the all-team panel call one core function. Revisit if a second league format or a lineup-rule change makes the divergence risky.

- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/15-all-team-rosters-panel.md`
  summary: `output$roster_table` (the operator roster panel, story 8/12) still name-indexes `view()$rosters[[st$user_team]]`. A legacy `state/draft.rds` with a blank or `NA` entry in `team_order` and `user_team` matching it passes `.warroom_validate_state()` (which checks `is.character`, length, and `anyDuplicated` only — one `""`/`NA` slips through) and then crashes the operator panel with "subscript out of bounds". Story 15 hardened the sibling all-team panel against exactly this by indexing `view()$rosters` positionally, but the operator panel was left as-is to avoid touching pre-existing story-8/12 code.
  evidence: Review finding (edge-case-hunter + blind-hunter + verification-gap, story 15). Pre-existing since story 8. Fix: either index the operator panel positionally too (`match(st$user_team, st$team_order)` with a guard) or add a `nzchar`/`anyNA` check on `team_order` entries to `.warroom_validate_state()` and document `user_team` as always a valid non-blank entry. Very low likelihood in this single-user local tool (team_order comes from `sprintf("Team %02d", ...)` or the terminal's validated order-entry flow); a hand-edited rds is the only vector.
...
- source_spec: `_bmad-output/specs/spec-fantasy-warroom/stories/17-panel-grid-layout.md`
  summary: The panel titles across app.R are all `<h4>` with no `<h1>`/`<h2>`/`<h3>` ancestor (`.app-header` is a plain `div` since story 9), so the document heading outline jumps straight to level 4 — a WCAG 1.3.1 / 2.4.6 smell. Story 17 restructured the layout into regions but left the heading levels untouched (pre-existing since story 8).
  evidence: Review finding (blind-hunter, story 17). Pre-existing, not caused by the layout change. The natural fix is to make `.app-header` an `<h1>` and the panel titles `<h2>` (and give `.workspace` / `.wide` / `.region--audit` a `role`/`aria-label` or wrap them in `<section>`), which belongs with story 23 (accessibility floor: aria-live region, combobox roles/labels, visible focus ring, Tab order). Several story-9..16 static tests assert the literal `h4("...")` strings, so those must move in lockstep.
